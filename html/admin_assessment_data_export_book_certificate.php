...
// Column names 
$fields = array('Registration No', 'National ID', 'Email', 'Name with initials', 'Full Name', 'Home address', 'Contact number', 'Institute', 'Field', 'Training Place', 'Training Place Address', 'Worksite', 'Worksite Address', 'Training Duration from', 'Training Duration to', 'Effective Date', 'Results', 'Assesment date');
...
if ($rs->num_rows > 0) {
    ...
    while ($d = $rs->fetch_assoc()) {
        $lineData = array($d["naita_id"], $d["nic"], $d["email"], $d["full_name_with_init"], $d["full_name"], $d["address"], $d["mobile_no"], $d["uni_name"], $d["fld_name"], $d["tran_plc_name"], $d["tran_plc_address"], $d["wrksit_name"], $d["wrksit_address"], $d["tran_est_from"], $d["tran_est_to"], $d["tran_efective_date"], $d["status"], $d["as_date"]);
